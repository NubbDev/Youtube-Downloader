...
async fn download_mp3(path: &str, link: &str) {
  ...
  let stream = video.best_audio()
    .unwrap();
  println!(format!("{}/{}.{}", path, video.title(), "mp3"));
  stream.download_to(format!("{}/{}.{}", path, video.title(), "mp3")).await.unwrap();
}
...
fn download(path: &str, link: &str, file: i16) {
  ...
  match file_type.as_str() {
    "mp3" => {
      tokio::runtime::Runtime::new().unwrap().block_on(download_mp3(path, link));
    }
    "mp4" => {},
    _ => {}
  }
