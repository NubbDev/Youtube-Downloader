...
//create a global variable to store the current progress
let mut current = 0;
...
async fn download_mp3(path: &str, link: &str) {
  ...
  path, 
    Callback::new().connect_on_progress_closure(|res| {
      let current = res.current_chunk as f64;
      let total = res.content_length.unwrap() as f64;
      println!("Downloaded {:?}%", (current / total * 100.0).floor());
